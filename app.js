...
nop`,
    ...
    // --- Reverse Engineering ---
    'xor-decode': `; XOR string decode - the #1 malware obfuscation trick
; 4 bytes are XORed with a single key (0x5A) to reveal "PASS".
; In memory the encoded bytes are: 0A 1B 09 09
mov esi, 0x3000              ; buffer address
mov dword ptr [esi], 0x09091B0A  ; store the 4 encoded bytes
mov ecx, 4                   ; number of bytes to decode
mov edi, esi                 ; cursor = start of buffer
decode_loop:
mov al, byte ptr [edi]      ; load one encoded byte
xor al, 0x5A                 ; apply the key (this is the whole trick)
mov byte ptr [edi], al      ; write the decoded byte back
inc edi                      ; advance to the next byte
dec ecx                      ; one fewer byte to go
jnz decode_loop              ; loop until ECX = 0
mov eax, dword ptr [esi]    ; EAX = 0x53534150
; Check the ASCII Table tool: 50='P' 41='A' 53='S' 53='S' -> "PASS"`,

    'keygen': `; Keygen challenge - reverse the serial check
; EAX holds the "entered" serial. The routine transforms it and
; compares against a target. ECX = 1 means ACCEPTED, 0 means REJECTED.
; The value below already passes - your job is to see WHY, then
; solve it from scratch by working backwards from the CMP.
mov eax, 42                  ; <-- the serial under test
imul eax, eax, 3             ; step 1: serial * 3
add eax, 16                  ; step 2: + 16
xor eax, 0xD4                ; step 3: XOR with 0xD4
cmp eax, 0x5A                ; gate: must equal the target 0x5A
je valid
mov ecx, 0                   ; ECX = 0  ->  REJECTED
jmp done
valid:
mov ecx, 1                   ; ECX = 1  ->  ACCEPTED
done:
nop
